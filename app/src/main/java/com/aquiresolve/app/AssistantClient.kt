...
/**
 * Cliente do Helô IA (plano 06). Fala SÓ com o nosso backend (`/api/ai/classify`), que
    ...
    suspend fun classify(description: String, niches: List<String>): Result = withContext(Dispatchers.IO) {
        val token = try {
            val user = FirebaseAuth.getInstance().currentUser
                ?: return@withContext Result.Error("Faça login para usar o Helô.")
        ...
        try {
            client.newCall(request).execute().use { response ->
                ...
                if (!response.isSuccessful || body == null) {
                    return@withContext Result.Error("Helô indisponível agora. Tente a busca.")
                }
            ...
            Result.Error("Sem conexão com o Helô. Tente a busca.")
        }
    }
}
