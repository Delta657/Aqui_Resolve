 ...
 * Cada linha mostra o nome do serviço, o nicho e o preço (de exibição, resolvido do catálogo).
 */
class ComboServiceItemAdapter(
    private val items: List<Row>
) : RecyclerView.Adapter<ComboServiceItemAdapter.RowViewHolder>() {

    data class Row(val name: String, val niche: String, val price: Double)
    ...
    override fun onBindViewHolder(holder: RowViewHolder, position: Int) {
        ...
        if (row.price > 0) {
            holder.price.text = String.format(Locale("pt", "BR"), "R$ %.2f", row.price)
