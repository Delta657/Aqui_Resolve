 ...
 * - Opções de avaliação e feedback
 */
class ClientChatActivity : AppCompatActivity() {
    ...
    private fun addWelcomeMessage() {
        val welcomeMessage = ChatMessage(
            ...
            senderId = "system",
            senderName = "Sistema",
            message = "Olá! Você está conversando com ${providerName ?: "o prestador"}. " +
                     "Aqui você pode esclarecer dúvidas sobre o serviço.",
            timestamp = Date(),
        ...
        db.collection("users").document(userId)
            .addSnapshotListener { doc, _ ->
                ...
                if (isOnline) {
                    ...
                    binding.tvProviderStatus.setTextColor(ContextCompat.getColor(this, R.color.success_color))
                } else {
                    val text = if (!lastSeenBrt.isNullOrEmpty()) "Visto às $lastSeenBrt" else "Offline"
                    binding.tvProviderStatus.text = text
                    binding.tvProviderStatus.setTextColor(ContextCompat.getColor(this, R.color.gray_500))
                }
            }
    }
    ...
    /**
     * Mostra diálogo de avaliação
     */
    private fun showRatingDialog() {
        // TODO: Implementar tela de avaliação
        Toast.makeText(this, "Avaliação será implementada em breve", Toast.LENGTH_SHORT).show()
    }

    /**
     * Mostra diálogo de denúncia
     */
    private fun showReportDialog() {
        val options = arrayOf("Comportamento inadequado", "Serviço de baixa qualidade", "Não compareceu", "Outro")
        
        AlertDialog.Builder(this)
            .setTitle("Denunciar Prestador")
            .setItems(options) { _, which ->
                val reason = options[which]
                // TODO: Implementar denúncia
                Toast.makeText(this, "Denúncia registrada: $reason", Toast.LENGTH_SHORT).show()
            }
    ...
    private fun showAttachmentOptions() {
        val options = arrayOf("📷 Tirar Foto", "🖼️ Galeria", "📄 Documento", "📍 Localização")
            ...
            chatManager.getMessagesFlow(oId).collectLatest { remoteMessages ->
                ...
                messages.addAll(remoteMessages.map { rm ->
                    ChatMessage(
                        ...
                        // Exibir do ponto de vista do CLIENTE
    ...
    private fun uploadImageToChat(imageUri: Uri, fileName: String) {
        lifecycleScope.launch {
            try {
                ...
                when (val result = imageManager.uploadImage(this@ClientChatActivity, uploadData)) {
