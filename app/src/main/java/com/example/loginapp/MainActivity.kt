...
class MainActivity : AppCompatActivity() {
    ...
    private fun requestNotificationPermissionIfNeeded(force: Boolean = false) {
        if (!com.example.loginapp.utils.PermissionHelper.needsNotificationPermission()) return
        if (com.example.loginapp.utils.PermissionHelper.isNotificationPermissionGranted(this)) return
