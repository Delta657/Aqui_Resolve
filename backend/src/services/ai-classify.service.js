...
function buildSystemPrompt(niches) {
  ...
  return [
    ...
    '- Se for claramente uma emergência (vazamento forte, cheiro de gás, curto), deixe a "message" tranquila, ágil e sem brincadeiras.',
    '',
    ...
    '- "message" deve ser curta (1 a 2 frases), acolhedora e em pt-BR. Pode soar natural, como: "Beleza, isso parece hidráulica. Posso te levar para Encanador?".',
...
async function classifyNiche({ description, niches }) {
  ...
  try {
    ...
    return { niche: null, confidence: 0, message: 'Não consegui identificar direitinho agora. Quer ver todos os serviços?' };
  }
  ...
  const message =
    typeof parsed?.message === 'string' && parsed.message.trim()
      ? parsed.message.trim()
      : niche
        ? `Beleza, isso parece um caso de ${niche}. Posso te direcionar?`
        : 'Não consegui identificar o serviço com segurança. Quer ver todos os serviços?';
