...
router.post('/classify', async (req, res, next) => {
  try {
    ...
    if (!description) {
      throw new HttpError(400, 'Descreva o que aconteceu para o Helô.', { code: 'AI_EMPTY_DESCRIPTION' });
    }
    if (niches.length === 0) {
    ...
    if (error.code === 'AI_NOT_CONFIGURED') {
      ...
      return next(new HttpError(503, 'Helô indisponível no momento.', { code: 'AI_NOT_CONFIGURED' }));
    }
    ...
    return next(new HttpError(502, 'Helô indisponível no momento.', { code: 'AI_UPSTREAM_ERROR' }));
  }
});
