...
// Ações suportadas pela Home do PRESTADOR (ver ProviderHomeActivity.onBannerClicked):
// niche → abre "Meus Nichos"; service → abre "Pedidos Disponíveis"; url → link externo.
// O prestador NÃO tem cashback/combos/parceiros, por isso essas ações não aparecem aqui.
const ACTION_TYPES: { value: string; label: string; hint: string }[] = [
  { value: "none", label: "Sem ação (apenas visual)", hint: "" },
  { value: "niche", label: "Abrir Meus Nichos", hint: "Qualquer texto não-vazio, ex.: Elétrica" },
  { value: "service", label: "Abrir Pedidos Disponíveis", hint: "" },
  { value: "url", label: "Abrir link externo", hint: "https://..." },
]

// service abre a tela de pedidos independentemente do valor; só niche e url precisam de destino.
function needsActionValue(actionType: string): boolean {
  return actionType === "niche" || actionType === "url"
}

export default function ProviderBannersConfigPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const res = await adminFetch("/api/provider-banners")
  ...
  const save = async () => {
    ...
    if (needsActionValue(form.actionType) && !form.actionValue.trim()) {
      toast({
        ...
        description: "Preencha o valor da ação (nicho ou URL).",
        variant: "destructive",
      })
      return
    }
    ...
    try {
      ...
      toast({ title: "Banner salvo!", description: "Já aparece na Home do prestador sem precisar de novo APK." })
      cancelForm()
      await load()
    } catch (e: unknown) {
      const message = e instanceof Error ? e.message : String(e)
  ...
  const remove = async (b: Banner) => {
    ...
    try {
      const res = await adminFetch(`/api/provider-banners?id=${encodeURIComponent(b.id)}`, { method: "DELETE" })
      const data = await res.json()
      if (!data.success) throw new Error(data.error || "Falha")
    ...
    <div className="space-y-6 p-4 md:p-6 max-w-4xl">
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold">Banners da Home do Prestador</h1>
            <p className="text-sm text-muted-foreground">
              Carrossel rotativo no topo da Home do prestador. Editar aqui reflete no app sem novo APK.
            </p>
          </div>
        </div>
        {!showForm && canCreate && (
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <div>
                ...
                <Input value={form.title} onChange={(e) => set({ title: e.target.value })} placeholder="Aumente seus ganhos" />
              </div>
              <div>
                ...
                <Input
                  ...
                  placeholder="Ative novos nichos hoje"
                />
              </div>
            </div>
      ...
      {loading ? (
        ...
        <Card>
          <CardContent className="flex flex-col items-center gap-2 py-10 text-center text-muted-foreground">
            ...
            <p>Nenhum banner cadastrado. Crie o primeiro para ele aparecer na Home do prestador.</p>
          </CardContent>
        </Card>
      ) : (
